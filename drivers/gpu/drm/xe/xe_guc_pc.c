...
static int pc_adjust_requested_freq(struct xe_guc_pc *pc)
{
	...
	if (pc->min_req_freq != 0) {
		ret = pc_set_min_freq(pc, pc->min_req_freq);
		if (ret)
			goto out;
	}

	if (pc->max_req_freq != 0) {
		ret = pc_set_max_freq(pc, pc->max_req_freq);
		if (ret)
			goto out;
	}
