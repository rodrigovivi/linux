 ...
 *    which applies it via BIOS Mailbox interface.
 * 5. Status is returned back to the host via MEI.
 */

#define GFX_SRV_MKHI_LB_CMD 0x12
#define GFX_SRV_MKHI_LB_RSP (GFX_SRV_MKHI_LB_CMD | 0x80)

#define LB_SEND_TIMEOUT_MSEC 3000
#define LB_RECV_TIMEOUT_MSEC 3000
...
static int mei_lb_check_response(const struct device *dev, const struct mkhi_msg_hdr *hdr)
{
	...
	if (hdr->command != GFX_SRV_MKHI_LB_RSP) {
		dev_err(dev, "Mismatch command: 0x%x instead of 0x%x\n",
			hdr->command, GFX_SRV_MKHI_LB_RSP);
		return -EINVAL;
	}
			       ...
			       const void *payload, size_t payload_size)
{
	...
	req->header.command = GFX_SRV_MKHI_LB_CMD;
	...
	bytes = mei_cldev_send_timeout(cldev,
				       (void *)req, req_size, LB_SEND_TIMEOUT_MSEC);
	if (bytes < 0) {
	...
	bytes = mei_cldev_recv_timeout(cldev,
				       (void *)&rsp, sizeof(rsp), LB_RECV_TIMEOUT_MSEC);
	if (bytes < 0) {
